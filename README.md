...
1. Course PDFs and reviews are chunked into semantic segments
2. Text is converted into vector embeddings
3. Relevant chunks are retrieved at query time
4. The LLM generates answers grounded only in retrieved context
...
2. Relevant agents retrieve:
- Course descriptions
- Prerequisites
- Student workload discussions
3. The planner agent evaluates dependency and overlap
4. The synthesis agent generates a grounded response
5. The user receives a clear, evidence-based answer
